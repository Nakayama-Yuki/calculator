...
export default function Calculator() {
   ...
   *
   * 注: 依存配列にハンドラー関数を含めています。これらの関数はuseCallbackでメモ化されており、
   * その依存配列に実際の状態（display, operator等）が含まれているため、
   * 状態が変更されるたびに関数が再作成され、useEffectが再実行されます。
   * これは外部システム（windowのkeydownイベント）との同期のための正しいパターンです。
   */
  useEffect(() => {
    // キーボードイベントハンドラー
  ...
  return (
    <div className="bg-(--calculator-bg) p-6 rounded-2xl shadow-2xl max-w-xs mx-auto transition-colors duration-300 ease-in-out">
